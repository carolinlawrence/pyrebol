...
class CDEC:
  ...
  def run_train(self):
    ...
    args = [self.config.cdec_extract,
            #'-a', '%s/gdfa' % (self.config.experiment_dir),   
...
density_prune=100' % (self.config.experiment_dir, self.config.tgt))
...
feature_function=KLanguageModel %s/%s.arpa\n\
feature_function=WordPenalty\n\
add_pass_through_rules=true' % (self.config.experiment_dir, self.config.tgt))
    if "mira" in self.config.weights:
      cdec_test.write('\nfeature_function=RuleIdentityFeatures \n\
feature_function=RuleSourceBigramFeatures \n\
feature_function=RuleTargetBigramFeatures \n\
...
scfg_max_span_limit=20')
  ...
  def decode_sentence(self, experiment_dir, sentence, temp_dir):
    ...
    args = [self.config.cdec_extract,
            '-a', '%s/model/aligned.%s' % (experiment_dir, self.config.symm),            
            '-f', '%s/%s.%s' % (experiment_dir, self.config.train_name, self.config.src),
            '-e', '%s/%s.%s' % (experiment_dir, self.config.train_name, self.config.tgt),
    ...
    infile = open('%s/sent.inline.tmp' % temp_dir, 'r')
