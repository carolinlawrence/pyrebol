...
def main():
    ...
    try:
        ...
        with open("%s.ref" % argparser.train) as f:
            reference = []
            for line in f:
                reference.append([line.strip()])  # a list of lists, where the first entry is always the gold reference
        f.close()
        ...
        for it in xrange(1, argparser.iterations + 1):
            ...
            for sent_counter, sent in enumerate(nl):
                ...
                # call to the decoder
                out_raw = decoder.translate_sentence("%s/decoder/cdec" % argparser.decoder, argparser.ini, weights_tmp,
